...
generateRiskReport({ refresh: process.argv.includes("--refresh") })
  .then((report) => {
    console.log(`NIST Control Retrieval Method: ${report.referenceProvenance.retrievalMethod}\n`);
    for (const risk of report.topRisks) {
      ...
      console.log(`NIST: ${risk.remediation.nistControl.id} ${risk.remediation.nistControl.name} (${risk.remediation.nistControl.retrievalMethod})`);
      console.log("");
    }
  })
  .catch((error) => {
